...
# 下边这个据说是twitter标准的，不过发现目前list也能用中文字符了。所以这个还没确定是不是这样。
# 还有个问题就是 \x80-\xff 是字符串编码后才用的，这个对unicode不管用。
#LIST_END_CHARS = ur'([a-z0-9_]{1,20})(/[a-z][a-z0-9\x80-\xFF-]{0,79})?'
# U+4E00..U+9FA5 CJK Unified Ideographs 1.1
LIST_END_CHARS = ur'([a-z0-9_]{1,20})(/[a-z0-9_\-\u4e00-\u9fa5]{1,79})?'
...
HASHTAG_EXP = ur'(^|%s+)(#|\uff03)([A-Z0-9_\u4e00-\u9fa5]+)' % SPACES
...
class Parser(object):
    ...
    def format_tag(self, tag, text):
        ...
        return '<a class="link-hashtag" href="/a/search?q=%s">%s%s</a>' \
                % (urllib.quote('#' + text.encode('utf-8')), tag, text)
    
    def format_username(self, at_char, user):
        '''Return formatted HTML for a username.'''
        return '<a class="link-screen_name" href="/t/%s">%s%s</a>' \
               % (user, at_char, user)
    
    def format_list(self, at_char, user, list_name):
        '''Return formatted HTML for a list.'''
        return '<a class="link-list" href="/t/%s/%s">%s%s/%s</a>' \
               % (user, list_name, at_char, user, list_name)
    
    def format_url(self, url, text):
        '''Return formatted HTML for a url.'''
        return '<a class="link-external" target="_blank" href="%s">%s</a>' % (escape(url), text)
